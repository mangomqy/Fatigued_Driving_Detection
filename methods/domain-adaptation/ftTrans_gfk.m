...
%% compute
if ~bSup
	Ps = pca(ftSrc);  % source subspace: sample * feature dimension
else % 利用source label降维
% 	addpath plslda
try
...
maxd = min(cellfun(@(x)size(x,2),{Ps,Pt,Pst})); % 子空间维数
Ps = Ps(:,1:maxd); % trial * dimension
Pt = Pt(:,1:maxd);
Pst = Pst(:,1:maxd);
% revised by yxy
...
d = min(d,floor(nFt/2)); % 最多为floor(dimension/2) 24

if dr ~= 0 % manually set the subspace dimension
	d = floor(min(maxd,floor(nFt/2))*dr); % 72
end
